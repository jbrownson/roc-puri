...
RoclayInternal := [].{
	...
	default_sizing = { width: Fit(RoclayInternal.unbounded), height: Fit(RoclayInternal.unbounded) }

	default_clip : Clip
	default_clip = { horizontal: Bool.False, vertical: Bool.False, child_offset: RoclayInternal.zero_point }

	default_box : BoxConfig
	default_box = {
		direction: LeftToRight,
		padding: RoclayInternal.zero_insets,
		gap: 0,
		sizing: RoclayInternal.default_sizing,
	...
	layout_tree = |root| {
		...
		aspect_heights = RoclayInternal.map_nodes(wrapped_text, RoclayInternal.scale_aspect_height)
		propagated_heights = RoclayInternal.propagate_resolved_heights(aspect_heights)
		after_vertical = RoclayInternal.size_along_axis(Vertical, propagated_heights)
	...
	map_nodes = |node, change| {
		...
		for child in node.children {
			$children = List.append($children, RoclayInternal.map_nodes(child, change))
		}
	...
	close_node = |node| {
		var $children = []
		for child in node.children {
			$children = List.append($children, RoclayInternal.close_node(child))
		}
		...
		closed = match node.content {
			...
			Container => RoclayInternal.close_container(with_children)
			Controlled(_) => RoclayInternal.close_container(with_children)
		}
	...
	close_container = |node| {
		...
		{
			..node,
			dimensions: RoclayInternal.expand_size_f32(node.config.padding, content_size),
			min_dimensions: RoclayInternal.close_container_min_dimensions(node.config, min_content_size),
		}
	}
	...
	container_content_size = |direction, gap, children, use_min| {
		...
		for child in children {
			...
			$primary = $primary + RoclayInternal.axis_size(primary_axis, size)
			$cross = F32.max($cross, RoclayInternal.axis_size(cross_axis, size))
		}
		with_gaps = $primary + RoclayInternal.gap_size(gap, List.len(children))
		RoclayInternal.size_from_axes(primary_axis, with_gaps, $cross)
	}
	...
	close_container_min_dimensions = |config, content_size| {
		...
		cross_axis = RoclayInternal.other_axis(primary_axis)
		primary = if RoclayInternal.box_clips_axis(primary_axis, config) {
			RoclayInternal.axis_padding(primary_axis, config.padding)
		} else {
			RoclayInternal.axis_size(primary_axis, content_size) + RoclayInternal.axis_padding(primary_axis, config.padding)
		}
		cross = if RoclayInternal.box_clips_axis(cross_axis, config) {
			0
		} else {
			RoclayInternal.axis_size(cross_axis, content_size) + RoclayInternal.axis_padding(cross_axis, config.padding)
		}
	...
	close_node_sizing = |node| {
		{
			..node,
			dimensions: {
				width: RoclayInternal.close_axis_size(node.config.sizing.width, node.dimensions.width),
				height: RoclayInternal.close_axis_size(node.config.sizing.height, node.dimensions.height),
			},
			min_dimensions: {
				width: RoclayInternal.close_axis_min_size(node.config.sizing.width, node.min_dimensions.width),
				height: RoclayInternal.close_axis_min_size(node.config.sizing.height, node.min_dimensions.height),
			},
		}
	}
	...
	close_axis_size = |sizing, value| match sizing {
		Percent(_) => 0
		_ => RoclayInternal.clamp_axis(sizing, value)
	}
	...
	clamp_axis = |sizing, value| {
		min_max = RoclayInternal.sizing_min_max(sizing)
	...
	sizing_min_max = |sizing| match sizing {
		...
		Percent(_) => RoclayInternal.unbounded
	}

	axis_min : AxisSizing -> Scalar
	axis_min = |sizing| match RoclayInternal.sizing_min_max(sizing).min {
		Bounded(value) => value
		Unbounded => 0
	}

	axis_max : AxisSizing -> Scalar
	axis_max = |sizing| match RoclayInternal.sizing_min_max(sizing).max {
		Bounded(value) => value
	...
	size_along_axis = |axis, parent| {
		...
		for child in direct_children {
			$children = List.append($children, RoclayInternal.size_along_axis(axis, child))
		}
	...
	size_children_along_axis = |axis, parent| {
		...
		axis_padding_value = RoclayInternal.axis_padding(axis, parent.config.padding)
		var $inner = 0
		var $index = 0
		for child in parent.children {
			child_sizing = RoclayInternal.node_axis_sizing(axis, child)
			child_size = RoclayInternal.axis_size(axis, child.dimensions)
			if primary {
				if $index > 0 {
		...
		total_padding_and_gaps = axis_padding_value + if primary RoclayInternal.gap_size(parent.config.gap, List.len(parent.children)) else 0

		var $percent_children = []
		var $inner_with_percent = $inner
		for child in parent.children {
			updated = match RoclayInternal.node_axis_sizing(axis, child) {
				Percent(percent) => {
					percent_size = (parent_size - total_padding_and_gaps) * percent
					if primary {
						$inner_with_percent = $inner_with_percent + percent_size
					}
					RoclayInternal.update_missing_aspect(RoclayInternal.update_node_axis_dimension(axis, percent_size, child))
				}
				_ => child
			}
		...
		if primary {
			...
			if remaining < 0 and !(RoclayInternal.node_clips_axis(axis, parent)) {
				RoclayInternal.distribute(axis, remaining, $percent_children, CompressNodes)
			} else if remaining > 0 {
				RoclayInternal.distribute(axis, remaining, $percent_children, GrowNodes)
			} else {
				$percent_children
			}
		} else {
			RoclayInternal.resolve_cross_axis(axis, parent, parent_size, axis_padding_value, $inner, $percent_children)
		}
	}
	...
	distribute = |axis, remaining, children, mode| {
		...
		var $index = 0
		for child in children {
			if RoclayInternal.distribution_active(mode, axis, child) {
	...
	distribute_active = |axis, remaining, children, mode, active_indices| {
		complete = match mode {
			...
			CompressNodes => remaining >= -RoclayInternal.epsilon
		}
		if complete {
			children
			...
			step = RoclayInternal.distribution_step(mode, axis, remaining, children, active_indices)
			...
			pass = RoclayInternal.apply_distribution_pass(mode, axis, step.frontier, amount, remaining, children, active_indices, 0)
			RoclayInternal.distribute_active(axis, pass.remaining, pass.children, mode, pass.active_indices)
		}
	}
	...
	distribution_step = |mode, axis, remaining, children, active_indices| match mode {
		GrowNodes => {
			...
			var $second_smallest = RoclayInternal.max_float
			var $width_to_add = remaining
			for child_index in active_indices {
				match List.get(children, child_index) {
					Ok(child) => {
						size = RoclayInternal.axis_size(axis, child.dimensions)
						if RoclayInternal.float_equal(size, $smallest) {
							{}
		...
		CompressNodes => {
			...
			for child_index in active_indices {
				match List.get(children, child_index) {
					Ok(child) => {
						size = RoclayInternal.axis_size(axis, child.dimensions)
						if RoclayInternal.float_equal(size, $largest) {
							{}
	...
	apply_distribution_pass = |mode, axis, frontier, amount, remaining, children, active_indices, position| {
		if position >= List.len(active_indices) {
			...
			Ok(child_index) => match List.get(children, child_index) {
				Err(_) => RoclayInternal.apply_distribution_pass(mode, axis, frontier, amount, remaining, children, active_indices, position + 1)
				Ok(child) => {
					previous = RoclayInternal.axis_size(axis, child.dimensions)
					if !(RoclayInternal.float_equal(previous, frontier)) {
						RoclayInternal.apply_distribution_pass(mode, axis, frontier, amount, remaining, children, active_indices, position + 1)
					} else {
						bound = match mode {
							GrowNodes => RoclayInternal.node_axis_max(axis, child)
							CompressNodes => RoclayInternal.axis_size(axis, child.min_dimensions)
						}
						resized = match mode {
							GrowNodes => F32.min(previous + amount, bound)
							CompressNodes => F32.max(previous + amount, bound)
						}
						next_children = RoclayInternal.replace_at(children, child_index, RoclayInternal.update_node_axis_dimension(axis, resized, child))
						...
						if at_bound {
							RoclayInternal.apply_distribution_pass(mode, axis, frontier, amount, next_remaining, next_children, RoclayInternal.remove_swapback_at(active_indices, position), position)
						} else {
							RoclayInternal.apply_distribution_pass(mode, axis, frontier, amount, next_remaining, next_children, active_indices, position + 1)
						}
					}
				}
			}
	...
	distribution_active = |mode, axis, child| if !(RoclayInternal.text_node_can_resize(child)) {
		...
		GrowNodes => match RoclayInternal.node_axis_sizing(axis, child) {
			Fill(_) => Bool.True
			_ => Bool.False
		}
		CompressNodes => match RoclayInternal.node_axis_sizing(axis, child) {
	...
	resolve_cross_axis = |axis, parent, parent_size, axis_padding_value, inner_size, children| {
		...
		var $next = []
		for child in children {
			updated = if !(RoclayInternal.text_node_can_resize(child)) {
				child
			} else {
				match RoclayInternal.node_axis_sizing(axis, child) {
					Fixed(_) => child
					Percent(_) => child
					Fit(_) => {
						resolved = F32.max(RoclayInternal.axis_size(axis, child.min_dimensions), F32.min(max_size, RoclayInternal.axis_size(axis, child.dimensions)))
						RoclayInternal.update_node_axis_dimension(axis, resolved, child)
					}
					Fill(_) => {
						resolved = F32.max(RoclayInternal.axis_size(axis, child.min_dimensions), F32.min(max_size, RoclayInternal.node_axis_max(axis, child)))
						RoclayInternal.update_node_axis_dimension(axis, resolved, child)
					}
				}
			}
	...
	scale_aspect_width = |node| if RoclayInternal.node_is_text(node) {
		...
		Some(ratio) => if ratio == 0 node else RoclayInternal.update_node_axis_dimension(Horizontal, ratio * node.dimensions.height, node)
		None => node
	}
	...
	propagate_resolved_heights = |node| if RoclayInternal.node_is_text(node) or List.len(node.children) == 0 {
		...
		var $children = []
		for child in node.children {
			$children = List.append($children, RoclayInternal.propagate_resolved_heights(child))
		}
		with_children = { ..node, children: $children }
		if RoclayInternal.direction_is_left_to_right(node.config.direction) {
			...
			{ ..with_children, dimensions: { ..with_children.dimensions, height: RoclayInternal.clamp_node_height(with_children, $height) } }
		} else {
			var $height = node.config.padding.top + node.config.padding.bottom + RoclayInternal.gap_size(node.config.gap, List.len($children))
			for child in $children {
	...
	clamp_node_height = |node, value| {
		...
		minimum = match node.config.sizing.height {
			...
			_ => RoclayInternal.node_axis_min(Vertical, node)
		}
		F32.min(RoclayInternal.node_axis_max(Vertical, node), F32.max(minimum, value))
	}
	...
	measure_text_node! = |config, string| {
		...
		for segment in RoclayInternal.segment_text(string) {
			word_size = if Str.is_empty(segment.word) RoclayInternal.zero_size else (config.measure!)(segment.word)
	...
	wrap_text_node = |available_width, text_node| {
		lines = match text_node.config.wrap_mode {
			TextWrapWords => if !(text_node.contains_newlines) and text_node.preferred_size.width <= available_width {
				...
				RoclayInternal.wrap_words(available_width, text_node.space_width, text_node.tokens)
			}
			TextWrapNewlines => RoclayInternal.wrap_newlines(text_node.tokens)
	...
	wrap_words = |available_width, space_width, tokens| {
		...
		for token in tokens {
			match token {
				TextNewline => {
					$lines = List.append($lines, RoclayInternal.trimmed_text_line($current_text, $current_width, space_width))
					$current_text = ""
					$current_width = 0
				}
				TextWord(word) => {
	...
	place_children! = |state, placement, node, child_offset| {
		...
		content_primary = RoclayInternal.axis_size(primary_axis, RoclayInternal.container_content_size(config.direction, config.gap, node.children, Bool.False))
		...
		for child in node.children {
			...
			available_cross = RoclayInternal.axis_size(cross_axis, RoclayInternal.shrink_size(config.padding, placement_size))
	...
	node_axis_max = |axis, node| match axis {
		...
		Vertical => match node.height_max_override {
			...
			None => match node.config.sizing.height {
				Percent(_) => 0
				other => RoclayInternal.axis_max(other)
			}
		}
	}
	...
	float_equal = |left, right| {
		...
		difference < RoclayInternal.epsilon and difference > -RoclayInternal.epsilon
	}
}
...
expect {
	...
	RoclayInternal.wrap_words(6, 1, tokens) == [
		{ text: "alpha", width: 5 },
		{ text: "beta", width: 4 },
		{ text: "gamma", width: 5 },
	]
...
expect {
	...
	RoclayInternal.wrap_newlines(tokens) == [{ text: "alpha", width: 5 }, { text: "beta", width: 4 }]
}
