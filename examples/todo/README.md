...
The app header references the four reusable packages through the same pinned,
content-addressed release URLs used by their adapters. The local RocRay facade
remains a sibling platform because it is specific to this example.
...
the Todo or local platform sources are newer than the executable.
