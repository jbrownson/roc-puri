...
PuriLineEditInternal := [].{
	...
	boundary_at_or_before = |bytes, requested| {
		...
		if index == 0 or index == length {
			...
			Ok(byte) => if PuriLineEditInternal.is_continuation_byte(byte) {
				PuriLineEditInternal.boundary_at_or_before(bytes, index - 1)
			} else {
				index
			}
			Err(_) => length
	...
	next_boundary_from = |bytes, index| if index >= List.len(bytes) {
		...
		Ok(byte) => if PuriLineEditInternal.is_continuation_byte(byte) {
			PuriLineEditInternal.next_boundary_from(bytes, index + 1)
		} else {
			index
		}
	...
	previous_boundary = |bytes, requested| {
		current = PuriLineEditInternal.boundary_at_or_before(bytes, requested)
		if current == 0 0 else PuriLineEditInternal.boundary_at_or_before(bytes, current - 1)
	}

	next_boundary : List(U8), U64 -> U64
	next_boundary = |bytes, requested| {
		current = PuriLineEditInternal.boundary_at_or_before(bytes, requested)
		if current >= List.len(bytes) List.len(bytes) else PuriLineEditInternal.next_boundary_from(bytes, current + 1)
	}
	...
	clamp_selection = |text, selection| {
		...
		{
			..selection,
			anchor: PuriLineEditInternal.boundary_at_or_before(bytes, selection.anchor),
			focus: PuriLineEditInternal.boundary_at_or_before(bytes, selection.focus),
		}
	}

	selection_bounds : Str, LineEditSelection -> SelectionBounds
	selection_bounds = |text, source| {
		selection = PuriLineEditInternal.clamp_selection(text, source)
		{
	...
	prefix = |source, requested| {
		...
		end = PuriLineEditInternal.boundary_at_or_before(bytes, requested)
	...
	slice = |source, requested_start, requested_end| {
		bytes = Str.to_utf8(source)
		first = PuriLineEditInternal.boundary_at_or_before(bytes, requested_start)
		second = PuriLineEditInternal.boundary_at_or_before(bytes, requested_end)
	...
	replace_range = |source, start, end, inserted| {
		bytes = Str.to_utf8(source)
		lo = PuriLineEditInternal.boundary_at_or_before(bytes, start)
		hi = PuriLineEditInternal.boundary_at_or_before(bytes, end)
		prefix_bytes = List.take_first(bytes, lo)
		suffix = List.drop_first(bytes, hi)
		with_insert = PuriLineEditInternal.append_bytes(prefix_bytes, Str.to_utf8(inserted))
		match Str.from_utf8(PuriLineEditInternal.append_bytes(with_insert, suffix)) {
			Ok(string) => string
			Err(_) => source
		}
	}
	...
	replace_selection = |text, inserted, source| {
		selection = PuriLineEditInternal.clamp_selection(text, source)
		bounds = PuriLineEditInternal.selection_bounds(text, selection)
		next_text = PuriLineEditInternal.replace_range(text, bounds.start, bounds.end, inserted)
	...
	selected_text = |text, selection| {
		bounds = PuriLineEditInternal.selection_bounds(text, selection)
		if bounds.start == bounds.end None else Some(PuriLineEditInternal.slice(text, bounds.start, bounds.end))
	}
	...
	delete_backward = |text, source| {
		...
		if bounds.start != bounds.end {
			PuriLineEditInternal.replace_selection(text, "", selection)
		} else if bounds.start == 0 {
			{ text, selection: { ..selection, anchor: 0, focus: 0, drag: NotDragging } }
		} else {
			bytes = Str.to_utf8(text)
			start = PuriLineEditInternal.previous_boundary(bytes, bounds.start)
			next_text = PuriLineEditInternal.replace_range(text, start, bounds.start, "")
	...
	delete_forward = |text, source| {
		...
		if bounds.start != bounds.end {
			...
			bytes = Str.to_utf8(text)
			end = PuriLineEditInternal.next_boundary(bytes, bounds.end)
			next_text = PuriLineEditInternal.replace_range(text, bounds.end, end, "")
	...
	class_at = |bytes, index| match List.get(bytes, index) {
		Ok(byte) => PuriLineEditInternal.byte_class(byte)
		Err(_) => SpaceByte
	}
	...
	scan_class_backward = |bytes, index, class| if index == 0 {
		...
		previous = PuriLineEditInternal.previous_boundary(bytes, index)
		if PuriLineEditInternal.same_class(PuriLineEditInternal.class_at(bytes, previous), class) {
			PuriLineEditInternal.scan_class_backward(bytes, previous, class)
		} else {
			index
		}
	}
	...
	scan_class_forward = |bytes, index, class| if index >= List.len(bytes) {
		List.len(bytes)
	} else if PuriLineEditInternal.same_class(PuriLineEditInternal.class_at(bytes, index), class) {
		PuriLineEditInternal.scan_class_forward(bytes, PuriLineEditInternal.next_boundary(bytes, index), class)
	} else {
		index
	}
	...
	word_bounds = |text, requested| {
		...
		index = if requested >= length and length > 0 {
			PuriLineEditInternal.previous_boundary(bytes, length)
		} else {
			PuriLineEditInternal.boundary_at_or_before(bytes, requested)
		}
		class = PuriLineEditInternal.class_at(bytes, index)
		{
			start: PuriLineEditInternal.scan_class_backward(bytes, index, class),
			end: PuriLineEditInternal.scan_class_forward(bytes, index, class),
		}
	}
	...
	previous_word_boundary_from = |bytes, index| if index == 0 {
		...
		previous = PuriLineEditInternal.previous_boundary(bytes, index)
		if PuriLineEditInternal.same_class(PuriLineEditInternal.class_at(bytes, previous), WordByte) {
			PuriLineEditInternal.scan_class_backward(bytes, index, WordByte)
		} else {
			PuriLineEditInternal.previous_word_boundary_from(bytes, previous)
		}
	}
	...
	previous_word_boundary = |text, requested| {
		bytes = Str.to_utf8(text)
		PuriLineEditInternal.previous_word_boundary_from(bytes, PuriLineEditInternal.boundary_at_or_before(bytes, requested))
	}

	next_word_boundary_from : List(U8), U64 -> U64
	next_word_boundary_from = |bytes, index| if index >= List.len(bytes) {
		List.len(bytes)
	} else if PuriLineEditInternal.same_class(PuriLineEditInternal.class_at(bytes, index), WordByte) {
		PuriLineEditInternal.scan_class_forward(bytes, index, WordByte)
	} else {
		PuriLineEditInternal.next_word_boundary_from(bytes, PuriLineEditInternal.next_boundary(bytes, index))
	}

	next_word_boundary : Str, U64 -> U64
	next_word_boundary = |text, requested| {
		bytes = Str.to_utf8(text)
		PuriLineEditInternal.next_word_boundary_from(bytes, PuriLineEditInternal.boundary_at_or_before(bytes, requested))
	}
	...
	move_focus = |text, source, direction, extend| {
		...
		moved = if !extend and bounds.start != bounds.end {
			...
			Backward => PuriLineEditInternal.previous_boundary(bytes, selection.focus)
			Forward => PuriLineEditInternal.next_boundary(bytes, selection.focus)
		}
		{
			..selection,
	...
	move_word = |text, source, direction, extend| {
		...
		moved = if !extend and bounds.start != bounds.end {
			...
			Backward => PuriLineEditInternal.previous_word_boundary(text, selection.focus)
			Forward => PuriLineEditInternal.next_word_boundary(text, selection.focus)
		}
		{
			..selection,
			anchor: if extend selection.anchor else moved,
	...
	delete_word_backward = |text, source| {
		...
		if bounds.start != bounds.end {
			...
			start = PuriLineEditInternal.previous_word_boundary(text, bounds.start)
			next_text = PuriLineEditInternal.replace_range(text, start, bounds.start, "")
			{ text: next_text, selection: { anchor: start, focus: start, drag: NotDragging } }
		}
	}
	...
	delete_word_forward = |text, source| {
		...
		if bounds.start != bounds.end {
			...
			end = PuriLineEditInternal.next_word_boundary(text, bounds.end)
			next_text = PuriLineEditInternal.replace_range(text, bounds.end, end, "")
			{ text: next_text, selection: { anchor: bounds.end, focus: bounds.end, drag: NotDragging } }
		}
	}
	...
	delete_to_start = |text, source| {
		...
		if bounds.start != bounds.end {
			...
			next_text = PuriLineEditInternal.replace_range(text, 0, bounds.start, "")
			{ text: next_text, selection: PuriLineEditInternal.empty_selection }
		}
	}
	...
	delete_to_end = |text, source| {
		...
		if bounds.start != bounds.end {
			...
			next_text = PuriLineEditInternal.replace_range(text, bounds.end, Str.count_utf8_bytes(text), "")
			{ text: next_text, selection: { anchor: bounds.end, focus: bounds.end, drag: NotDragging } }
		}
	}
	...
	start_pointer_selection = |text, source, requested, clicks, extend| if clicks >= 3 {
		...
		bounds = PuriLineEditInternal.word_bounds(text, requested)
		{ anchor: bounds.start, focus: bounds.end, drag: WordDrag(bounds) }
	} else {
		index = PuriLineEditInternal.boundary_at_or_before(Str.to_utf8(text), requested)
		selection = PuriLineEditInternal.clamp_selection(text, source)
	...
	continue_drag = |text, source, requested| {
		...
		match selection.drag {
			...
			WordDrag(origin) => {
				target = PuriLineEditInternal.word_bounds(text, index)
	...
	handle_key = |text, selection, event| match event.state {
		...
		KeyDown => match event.key {
			Character(string) if (event.modifiers.meta or event.modifiers.ctrl) and Str.caseless_ascii_equals(string, "a") => Edited({ text, selection: PuriLineEditInternal.select_all(text) })
			Character(string) if (event.modifiers.meta or event.modifiers.ctrl) and Str.caseless_ascii_equals(string, "c") => match PuriLineEditInternal.selected_text(text, selection) {
				Some(selected) => Copy(selected)
				None => Ignored
			}
			Character(string) if (event.modifiers.meta or event.modifiers.ctrl) and Str.caseless_ascii_equals(string, "x") => match PuriLineEditInternal.selected_text(text, selection) {
				Some(selected) => Cut({ copied: selected, edit: PuriLineEditInternal.replace_selection(text, "", selection) })
				None => Ignored
			}
			...
			Character(string) => if event.modifiers.ctrl or event.modifiers.meta {
				...
				Edited(PuriLineEditInternal.replace_selection(text, string, selection))
			}
			Named(Backspace) => if event.modifiers.meta {
				Edited(PuriLineEditInternal.delete_to_start(text, selection))
			} else if event.modifiers.alt or event.modifiers.ctrl {
				Edited(PuriLineEditInternal.delete_word_backward(text, selection))
			} else {
				Edited(PuriLineEditInternal.delete_backward(text, selection))
			}
			Named(Delete) => if event.modifiers.meta {
				Edited(PuriLineEditInternal.delete_to_end(text, selection))
			} else if event.modifiers.alt or event.modifiers.ctrl {
				Edited(PuriLineEditInternal.delete_word_forward(text, selection))
			} else {
				Edited(PuriLineEditInternal.delete_forward(text, selection))
			}
			Named(ArrowLeft) => if event.modifiers.meta {
				Edited({ text, selection: PuriLineEditInternal.move_home(selection, event.modifiers.shift) })
			} else if event.modifiers.alt or event.modifiers.ctrl {
				Edited({ text, selection: PuriLineEditInternal.move_word(text, selection, Backward, event.modifiers.shift) })
			} else {
				Edited({ text, selection: PuriLineEditInternal.move_focus(text, selection, Backward, event.modifiers.shift) })
			}
			Named(ArrowRight) => if event.modifiers.meta {
				Edited({ text, selection: PuriLineEditInternal.move_end(text, selection, event.modifiers.shift) })
			} else if event.modifiers.alt or event.modifiers.ctrl {
				Edited({ text, selection: PuriLineEditInternal.move_word(text, selection, Forward, event.modifiers.shift) })
			} else {
				Edited({ text, selection: PuriLineEditInternal.move_focus(text, selection, Forward, event.modifiers.shift) })
			}
			Named(Home) => Edited({ text, selection: PuriLineEditInternal.move_home(selection, event.modifiers.shift) })
			Named(End) => Edited({ text, selection: PuriLineEditInternal.move_end(text, selection, event.modifiers.shift) })
			Named(Space) => if event.modifiers.ctrl or event.modifiers.meta Ignored else Edited(PuriLineEditInternal.replace_selection(text, " ", selection))
			_ => Ignored
		}
	}
}
...
expect {
	first = PuriLineEditInternal.delete_backward("a🐦b", PuriLineEditInternal.selection_at_end("a🐦b"))
	second = PuriLineEditInternal.delete_backward(first.text, first.selection)
...
expect {
	...
	replaced = PuriLineEditInternal.replace_selection("hello", "i", selection)
	replaced.text == "hio" and replaced.selection.anchor == 2 and replaced.selection.focus == 2 and !(PuriLineEditInternal.is_dragging(replaced.selection))
}

expect {
	selection = { anchor: 1, focus: 4, drag: NotDragging }
	left = PuriLineEditInternal.move_focus("hello", selection, Backward, Bool.False)
	right = PuriLineEditInternal.move_focus("hello", selection, Forward, Bool.False)
	left.anchor == 1 and left.focus == 1 and right.anchor == 4 and right.focus == 4
}

expect {
	text = "one, two  three"
	bounds = PuriLineEditInternal.word_bounds(text, 6)
	backward = PuriLineEditInternal.move_word(text, { anchor: 15, focus: 15, drag: NotDragging }, Backward, Bool.False)
	forward = PuriLineEditInternal.move_word(text, PuriLineEditInternal.empty_selection, Forward, Bool.False)
	bounds.start == 5 and bounds.end == 8 and backward.focus == 10 and forward.focus == 3
}

expect {
	text = "one, two"
	backward = PuriLineEditInternal.delete_word_backward(text, PuriLineEditInternal.selection_at_end(text))
	forward = PuriLineEditInternal.delete_word_forward(text, PuriLineEditInternal.empty_selection)
	backward.text == "one, " and backward.selection.focus == 5 and forward.text == ", two" and forward.selection.focus == 0
}

expect {
	text = "one two"
	double = PuriLineEditInternal.start_pointer_selection(text, PuriLineEditInternal.empty_selection, 5, 2, Bool.False)
	extended = PuriLineEditInternal.continue_drag(text, double, 1)
	triple = PuriLineEditInternal.start_pointer_selection(text, PuriLineEditInternal.empty_selection, 5, 3, Bool.False)
...
expect {
	...
	selected = PuriLineEditInternal.handle_key("hello", selection, select_event)
	copied = PuriLineEditInternal.handle_key("hello", { anchor: 1, focus: 4, drag: NotDragging }, copy_event)
