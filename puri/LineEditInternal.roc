...
LineEditInternal := [].{
	...
	class_at = |bytes, index| match List.get(bytes, index) {
		Ok(byte) => LineEditInternal.byte_class(byte)
		Err(_) => SpaceByte
	}
	...
	word_bounds = |text, requested| {
		...
		{
			start: LineEditInternal.scan_class_backward(bytes, index, class),
			end: LineEditInternal.scan_class_forward(bytes, index, class),
		}
	}
	...
	previous_word_boundary_from = |bytes, index| if index == 0 {
		...
		if LineEditInternal.same_class(LineEditInternal.class_at(bytes, previous), WordByte) {
			...
			LineEditInternal.previous_word_boundary_from(bytes, previous)
		}
	}
	...
	move_word = |text, source, direction, extend| {
		...
		moved = if !extend and bounds.start != bounds.end {
			...
			Forward => LineEditInternal.next_word_boundary(text, selection.focus)
		}
		{
			..selection,
			anchor: if extend selection.anchor else moved,
...
expect {
	first = LineEditInternal.delete_backward("a🐦b", LineEditInternal.selection_at_end("a🐦b"))
	second = LineEditInternal.delete_backward(first.text, first.selection)
...
expect {
	...
	replaced = LineEditInternal.replace_selection("hello", "i", selection)
	replaced.text == "hio" and replaced.selection.anchor == 2 and replaced.selection.focus == 2 and !(LineEditInternal.is_dragging(replaced.selection))
}

expect {
	selection = { anchor: 1, focus: 4, drag: NotDragging }
	left = LineEditInternal.move_focus("hello", selection, Backward, Bool.False)
	right = LineEditInternal.move_focus("hello", selection, Forward, Bool.False)
...
expect {
	text = "one, two"
	backward = LineEditInternal.delete_word_backward(text, LineEditInternal.selection_at_end(text))
	forward = LineEditInternal.delete_word_forward(text, LineEditInternal.empty_selection)
	backward.text == "one, " and backward.selection.focus == 5 and forward.text == ", two" and forward.selection.focus == 0
}

expect {
	text = "one two"
	double = LineEditInternal.start_pointer_selection(text, LineEditInternal.empty_selection, 5, 2, Bool.False)
	extended = LineEditInternal.continue_drag(text, double, 1)
	triple = LineEditInternal.start_pointer_selection(text, LineEditInternal.empty_selection, 5, 3, Bool.False)
