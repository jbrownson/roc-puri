app [main!] {
	test_host: platform "./platform/main.roc",
	geometry: "../../geometry/main.roc",
	puri: "../main.roc",
	recording: "./support/main.roc",
}

import geometry.Geometry2d
import puri.Puri
import puri.PuriCanvas
import puri.PuriEvent
import recording.PuriCanvasRecording
import puri.PuriHandler
import puri.PuriInteract

down_at : F32, F32 -> PuriEvent.PointerButtonEvent
down_at = |x, y| {
	position: Geometry2d.point(x, y),
	button: Some(Primary),
	clicks: 1,
	modifiers: PuriEvent.empty_modifiers,
}

double_down_at : F32, F32 -> PuriEvent.PointerButtonEvent
double_down_at = |x, y| { ..down_at(x, y), clicks: 2 }

frame_plus_composes_result_and_handler! : () => Bool
frame_plus_composes_result_and_handler! = || {
	canvas : PuriCanvas.Canvas(PuriCanvasRecording.Recording(Str), Str)
	canvas = PuriCanvasRecording.canvas
	outer = PuriHandler.on_event(|value, _event| Handled(value + 1))
	child = PuriHandler.on_event(|value, _event| Handled(value + 10))
	empty_frame : Puri.Frame(PuriCanvasRecording.Recording(Str), U64, [PointerDown(PuriEvent.PointerButtonEvent)])
	empty_frame = Puri.Frame.default()
	outer_frame = Puri.register(outer, empty_frame)
	child_frame = Puri.register(child, Puri.frame((canvas.fill_rect!)(Geometry2d.rect(0, 0, 5, 5), "child")))
	frame = List.sum([outer_frame, child_frame])
	result = PuriHandler.dispatch!(frame.handler, 0, PointerDown(down_at(1, 1)))
	List.len(frame.result.commands) == 1 and result == Handled(10)
}

clickable_uses_settled_rect! : () => Bool
clickable_uses_settled_rect! = || {
	placement = Geometry2d.root_placement(Geometry2d.rect(10, 10, 20, 10))
	clickable : Puri.Widget(PuriCanvasRecording.Recording(Str), U64, PuriInteract.Events(events))
	clickable = PuriInteract.clickable(|value| value + 1)
	frame = clickable(placement)
	outside = PuriHandler.dispatch!(frame.handler, 0, PointerDown(down_at(9, 15)))
	inside = PuriHandler.dispatch!(frame.handler, 0, PointerDown(down_at(12, 15)))
	outside == Declined and inside == Handled(1)
}

double_click_overrides_single_click_on_second_press! : () => Bool
double_click_overrides_single_click_on_second_press! = || {
	placement = Geometry2d.root_placement(Geometry2d.rect(10, 10, 20, 10))
	clickable : Puri.Widget(PuriCanvasRecording.Recording(Str), U64, PuriInteract.Events(events))
	clickable = PuriInteract.clickable(|value| value + 1)
	double_clickable : Puri.Widget(PuriCanvasRecording.Recording(Str), U64, PuriInteract.Events(events))
	double_clickable = PuriInteract.double_clickable(|value| value + 10)
	frame = clickable(placement) + double_clickable(placement)
	single = PuriHandler.dispatch!(frame.handler, 0, PointerDown(down_at(12, 15)))
	double = PuriHandler.dispatch!(frame.handler, 0, PointerDown(double_down_at(12, 15)))
	single == Handled(1) and double == Handled(10)
}

main! = || if frame_plus_composes_result_and_handler!() and clickable_uses_settled_rect!() and double_click_overrides_single_click_on_second_press!() 0 else 1
