...
focused_edit_draws_caret_and_dispatches! = || {
	...
	interaction = Focused({ selection, change!, blur! })
	...
	blurred = PuriHandler.dispatch_key!(frame.handler, model, enter_event)
	...
	List.len(frame.render.commands) == 2 and typed_correctly and blurred_correctly
}
...
main! = || if unfocused_click_focuses_at_measured_caret!() and focused_edit_draws_caret_and_dispatches!() and selection_draws_behind_text_and_caret!() 0 else 1
